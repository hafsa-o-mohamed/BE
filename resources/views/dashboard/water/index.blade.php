        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($waterBills as $bill)
                                    <tr class="{{ $bill->current_balance < 0 ? 'bg-red-100' : 'bg-green-100' }}">
                                        ...
                                        <td class="px-6 py-4">{{ $bill->current_balance }}</td>
                    ...
                    @else
                        <p class="text-center text-gray-500 py-4">لا توجد فواتير مياه</p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <h3 class="text-lg leading-6 font-medium text-gray-900 text-right mb-4">إضافة فاتورة جديدة</h3>
                    <form action="{{ route('water-bills.store') }}" method="POST">
                    @csrf
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" step="0.01" name="default_balance" id="default_balance" value="500" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" readonly>
                    </div>
                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2 text-right" for="current_balance">
                            الرصيد الحالي
                        </label>
                        <input type="number" step="0.01" name="current_balance" id="current_balance" value="500" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" readonly>
                    </div>
                    <div class="mb-4">
...
<script>
    ...
    function calculateCurrentBalance() {
        const defaultBalance = 500; // Hard-coded default balance
    ...
    function fetchLastWaterBill(buildingId) {
        fetch(`/water-bills/last?building_id=${buildingId}`)
            .then(response => response.json())
            .then(data => {
                document.getElementById('default_balance').value = 500; // Always set default balance to 500
            ...
            .catch(error => console.error('Error fetching last water bill:', error));
    }
    ...
    window.addEventListener('load', function() {
        ...
        if (buildingSelect) {
            // Fetch initial water bill data when modal opens
            buildingSelect.addEventListener('change', function() {
                fetchLastWaterBill(this.value);
            });
            
            // Trigger initial fetch for the first selected building
            if (buildingSelect.value) {
                fetchLastWaterBill(buildingSelect.value);
            }
        }
